...
export default function UserRegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-dark-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 left-1/4 w-[280px] h-[280px] bg-primary-100/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      >
        <div className="bg-dark-800 border border-dark-600/35 rounded-3xl shadow-card overflow-hidden">
          ...
          <div className="p-8 text-center border-b border-dark-600/10 bg-dark-900/20">
            ...
            <h1 className="font-display text-2xl font-bold text-dark-400 tracking-tight">Create Account</h1>
            <p className="text-gray-500 text-xs mt-1">Join us to rate and review our projects</p>
          </div>
          ...
          <div className="p-8">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-widest text-left">Full Name</label>
                <div className="relative group">
                  <User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-gold-400 transition-colors" />
                  <input id="reg-name" type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" className="input-field pl-10" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input id="reg-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" className="input-field pl-10" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-widest text-left">Confirm Password</label>
                <div className="relative group">
                  <Lock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-gold-400 transition-colors" />
                  <input id="reg-confirm" type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" className="input-field pl-10" />
                </div>
              </div>

              <button type="submit" disabled={loading} className="w-full btn-primary justify-center py-3 text-sm mt-2 disabled:opacity-70 disabled:cursor-not-allowed">
                {loading ? (
            ...
            <div className="text-center mt-6 space-y-3">
              <p className="text-gray-500 text-xs">
                ...
                <Link to="/login" state={{ from }} className="text-gold-500 font-semibold hover:text-gold-400 transition-colors">
                  Sign in
                </Link>
              </p>
